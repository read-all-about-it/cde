(ns cde.jwt-auth
  ...
  Provides functions for verifying Auth0 JWT tokens using RS256 algorithm.
  Keys are fetched from Auth0's JWKS endpoint and cached for performance.
  ...
  See also: [[cde.middleware/wrap-auth0]]"
  (:require
...
(def jwks-url
  "Auth0 JWKS endpoint URL for fetching public keys."
  "https://read-all-about-it.au.auth0.com/.well-known/jwks.json")

(def ^:private jwks-cache
  "Atom storing cached JWKS keys and fetch timestamp.

  Structure: `{:keys [jwk-maps...] :fetched-at epoch-ms}`"
  (atom {:keys nil :fetched-at 0}))

(def ^:private cache-duration-ms
  "Duration to cache JWKS keys before refetching (10 minutes)."
  (* 10 60 1000))

(defn ^:no-doc base64url-decode
  "Decodes a base64url-encoded string to bytes.

  Base64url differs from standard base64 by using `-` instead of `+`
  and `_` instead of `/`. Handles missing padding automatically.
...
(defn ^:no-doc jwk->public-key
  "Converts a JWK map to a Java RSA PublicKey.

  Extracts the modulus (`n`) and exponent (`e`) from the JWK and
  constructs an RSA public key suitable for signature verification.

  Arguments:
  - `jwk` - JWK map with `:n` and `:e` keys (base64url-encoded)

  Returns: Java PublicKey instance, or nil on error."
  [jwk]
  (try
...
(defn fetch-jwks
  "Fetches JWKS from Auth0 endpoint with caching.

  Returns cached keys if still valid (within [[cache-duration-ms]]).
  On fetch failure, returns stale cached keys if available.

  Returns: Vector of JWK maps, or nil if unavailable."
  []
...
(defn verify-jwt
  ...
  1. Parses JWT header to extract key ID (`kid`) and algorithm
  2. Validates algorithm is RS256
  3. Fetches matching public key from JWKS
  4. Verifies cryptographic signature using buddy-sign
  5. Validates expiration, issuer, and audience claims

  Arguments:
  - `token` - JWT token string (three dot-separated parts)

  Returns: Claims map if valid, nil if verification fails.

  Expected claims:
  - `:sub` - Auth0 user identifier
  - `:email` - User email address
  - `:exp` - Expiration timestamp
  - `:iss` - Must be Auth0 issuer URL
  - `:aud` - Must include API audience URL"
  [token]
  (try
    ;; Parse JWT parts
