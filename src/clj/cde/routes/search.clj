(ns cde.routes.search
  ...
  Provides search endpoints for titles, chapters, and newspapers.
  Supports filtering by multiple criteria with fuzzy matching
  (Levenshtein distance) and ILIKE pattern matching.

  See also: [[cde.db.search]] for search implementation."
  (:require
...
(defn search-routes
  "Returns search-related route definitions.

  Routes:
  - `GET /search/titles` - Search titles with fuzzy matching
  - `GET /search/chapters` - Search chapter content
  - `GET /search/newspapers` - Search newspapers by Trove ID
