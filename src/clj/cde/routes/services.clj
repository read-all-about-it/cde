(ns cde.routes.services
  "Main API routes composition and Swagger configuration.

  Assembles all domain-specific route modules into the `/api/v1` router
  and configures middleware for content negotiation, coercion, and
  parameter handling.

  Route modules included:
  - [[cde.routes.auth]]: Authentication endpoints
  - [[cde.routes.platform]]: Platform statistics
  - [[cde.routes.search]]: Full-text search
  - [[cde.routes.newspaper]]: Newspaper CRUD
  - [[cde.routes.author]]: Author CRUD
  - [[cde.routes.title]]: Title CRUD
  - [[cde.routes.chapter]]: Chapter CRUD
  - [[cde.routes.trove]]: Trove API proxy

  Swagger documentation available at `/api/swagger.json` and `/swagger-ui`."
  (:require
...
(defn service-routes
  "Composes all domain-specific routes into the main API router.

  Configures the `/api` route tree with:
  - Spec coercion for request/response validation
  - Muuntaja content negotiation (JSON, Transit, EDN)
  - Swagger documentation generation
  - Multipart file upload support

  Returns: Reitit route data structure for use with [[cde.handler/app-routes]]."
  []
  ["/api"
