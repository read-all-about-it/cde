(ns cde.routes.author
  "Author entity API routes (CRUD operations).

  Provides endpoints for creating, reading, updating, and listing authors.
  Create and update operations require JWT authentication.

  See also: [[cde.db.author]] for database operations."
  (:require
...
(defn author-routes
  "Returns author-related route definitions.

  Routes:
  - `GET /authors` - List authors (paginated)
  - `GET /author/:id` - Get author by ID
  - `PUT /author/:id` - Update author (auth required)
  - `GET /author/:id/titles` - Get titles by author
  - `GET /author-nationalities` - List unique nationalities
  - `GET /author-genders` - List unique genders
  - `POST /create/author` - Create author (auth required)
