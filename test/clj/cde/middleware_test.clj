...
(deftest test-extract-user-from-jwt
  (testing "Extract user from valid JWT claims"
    (let [request {:jwt-claims {"sub" "auth0|123"
                                 "email" "test@example.com"
                                 "name" "Test User"}}
          user (mw/extract-user-from-jwt request)]
      (is (= "auth0|123" (:user-id user)))
      (is (= "test@example.com" (:email user)))
      (is (= "Test User" (:name user)))))

  (testing "Extract user from request without JWT claims"
    (let [request {}
          user (mw/extract-user-from-jwt request)]
      (is (nil? user))))

  (testing "Extract user with partial claims"
    (let [request {:jwt-claims {"sub" "auth0|123"}}
          user (mw/extract-user-from-jwt request)]
      (is (= "auth0|123" (:user-id user)))
      (is (nil? (:email user)))
...
(deftest test-check-auth0-cookie-deprecated
  (testing "Cookie-based auth is marked as deprecated"
    (let [handler-source (str mw/check-auth0-cookie)]
      (is (re-find #"DEPRECATED" handler-source))))

  (testing "Cookie check still works but should not be used"
    (let [handler (mw/check-auth0-cookie mock-handler)
          request (-> (mock/request :get "/test")
                      (assoc-in [:headers "cookie"]
                                "auth0.brtvKymeXTTht8IwrQdhwYirSxt65K95.is.authenticated=true"))]
      ;; This test documents the deprecated behavior
      ;; In production, this should NOT be relied upon
      (let [response (handler request)]
...
(deftest test-middleware-ordering
  (testing "Middleware stack order is correct"
    ;; This test ensures middleware is applied in the correct order
    ;; JWT validation should happen before authorization checks
    (let [request (mock/request :get "/api/v1/create/author")]
      ;; Without JWT token, should fail at JWT validation, not later
      (is (thrown-with-msg? Exception #"401|Authentication"
                            (str "Request should fail at JWT validation"))))))
(ns cde.middleware-test)